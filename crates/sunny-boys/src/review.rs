...
mod tests {
    ...
    impl LlmProvider for FailingProvider {
        ...
        async fn chat(&self, _req: LlmRequest) -> Result<LlmResponse, LlmError> {
            Err(LlmError::InvalidResponse {
                message: "review llm unavailable".to_string(),
            })
        }
    }
    ...
    #[tokio::test]
    async fn test_review_fails_when_provider_configured_but_dead() {
        let agent = ReviewAgent::new(Some(Arc::new(FailingProvider)));
        let err = agent
            .handle_message(mk_msg("fn main() {}"), &mk_ctx())
            .await
        ...
        match err {
            AgentError::ExecutionFailed { source } => {
                assert_eq!(
                    source.to_string(),
                    "invalid response from provider: review llm unavailable"
                );
            }
            AgentError::Timeout => panic!("expected execution error, got timeout"),
            AgentError::NotFound { id } => {
